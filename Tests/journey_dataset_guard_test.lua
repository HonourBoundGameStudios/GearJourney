...
-- pair publishes the SAME global (GearJourney_ItemIndex / _AtlasItems), so under
...
local function loadCompat(projectId, mainline)
  ...
  _G.GearJourney_Engine = nil      -- avoid the ApplyRetail auto-call at load
...
do
  ...
  local idx, g = load("JourneyItemIndex_Retail.lua", "GearJourney_ItemIndex")
  H.ok(#idx > 0 and g ~= nil, "retail index populates on retail")
  local idx2, g2 = load("JourneyItemIndex.lua", "GearJourney_ItemIndex")
  H.ok(next(idx2) == nil and g2 == nil, "classic index no-ops on retail")
  local pool, pg = load("JourneyAtlasData_Retail.lua", "GearJourney_AtlasItems")
  H.ok(#pool > 0 and pg ~= nil, "retail pool populates on retail")
  local pool2, pg2 = load("JourneyAtlasData.lua", "GearJourney_AtlasItems")
...
do
  ...
  local idx, g = load("JourneyItemIndex.lua", "GearJourney_ItemIndex")
  H.ok(#idx > 0 and g ~= nil, "classic index populates on classic")
  local idx2, g2 = load("JourneyItemIndex_Retail.lua", "GearJourney_ItemIndex")
  H.ok(next(idx2) == nil and g2 == nil, "retail index no-ops on classic")
  local pool, pg = load("JourneyAtlasData.lua", "GearJourney_AtlasItems")
  H.ok(#pool > 0 and pg ~= nil, "classic pool populates on classic")
  local pool2, pg2 = load("JourneyAtlasData_Retail.lua", "GearJourney_AtlasItems")
  H.ok(next(pool2) == nil and pg2 == nil, "retail pool no-ops on classic")
end

-- ── No Compat (offline default): guard is skipped, files load fully ──────────
do
  _G.GearJourney_Compat = nil
  local idx = load("JourneyItemIndex.lua", "GearJourney_ItemIndex")
